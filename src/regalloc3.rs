...
use regalloc3::{
    ...
    reginfo::{
        AllocationOrderSet, PhysReg, RegBank, RegClass, RegClassSet, RegGroup, RegInfo,
        RegOrRegGroup, RegOrRegGroupSet, RegUnit, SpillSlotSize,
    },
...
impl RegInfo for RegInfoImpl {
    ...
    #[inline]
    fn class_members(&self, class: RegClass) -> regalloc3::reginfo::RegOrRegGroupSet {
        self.classes[class].regs.map_index()
    }
    ...
    fn build(&mut self, func: &impl crate::Function, mach_env: &MachineEnv) {
        ...
        for class in [
                ...
                && mach_env.non_preferred_regs_by_class[class as usize].is_empty()
            {
                ...
                SpillSlotSize::new(func.spillslot_size(class) as u32)
            };
            ...
            self.classes.push(RegClassData {
                regs: RegOrRegGroupSet::from_iter(
                    preferred_regs()
                ...
                preferred_regs: preferred_regs().map(RegOrRegGroup::single).collect(),
                non_preferred_regs: non_preferred_regs().map(RegOrRegGroup::single).collect(),
            });
            // Reg
            self.classes.push(RegClassData {
                regs: RegOrRegGroupSet::from_iter(
