...
use regalloc3::{Options, SplitStrategy};

use crate::cfg::CFGInfoCtx;
use crate::{cfg::CFGInfo, ion::Stats, Edit, MachineEnv, Output, PReg, PRegSet, ProgPoint, VReg};

// We create CLASS_PER_BANK register classes in 3 banks
const CLASS_PER_BANK: usize = 8;
...
impl ClassKind {
    fn from_class(class: RegClass) -> Self {
        match class.index() % CLASS_PER_BANK {
            0 => Self::Any,
            1 => Self::Reg,
            2 => Self::Stack,
            3 => Self::Limit32,
    ...
    fn to_class(self, bank: RegBank) -> RegClass {
        RegClass::new(bank.index() * CLASS_PER_BANK + self as usize)
    }
}
...
impl RegInfo for RegInfoImpl {
    ...
    fn bank_for_class(&self, class: RegClass) -> RegBank {
        RegBank::new(class.index() / CLASS_PER_BANK)
    }
    ...
    fn class_includes_spillslots(&self, class: RegClass) -> bool {
        matches!(
            ClassKind::from_class(class),
            ClassKind::Any | ClassKind::Stack
        )
    }
    ...
    fn class_spill_cost(&self, class: RegClass) -> f32 {
        match ClassKind::from_class(class) {
            ...
            ClassKind::Reg => 1.0,
            ClassKind::Stack => 0.0,
            ClassKind::Limit32 => 1.0,
            ClassKind::Limit16 => 1.0,
            ClassKind::Limit8 => 1.0,
            ClassKind::Limit4 => 1.0,
            ClassKind::Limit2 => 1.0,
        }
    }
        ...
        for class in [
            ...
            let regs = |limit| {
                mach_env.preferred_regs_by_class[class as usize]
                    .iter()
                    .chain(&mach_env.non_preferred_regs_by_class[class as usize])
                    .filter(move |preg| preg.hw_enc() < limit)
            ...
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(!0).chain(stack_regs())),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Any.to_class(bank),
                    ClassKind::Reg.to_class(bank),
                    ClassKind::Stack.to_class(bank),
                    ClassKind::Limit32.to_class(bank),
                    ClassKind::Limit16.to_class(bank),
                    ClassKind::Limit8.to_class(bank),
                    ClassKind::Limit4.to_class(bank),
                    ClassKind::Limit2.to_class(bank),
                ]),
                allocation_order: regs(!0).collect(),
            });
            // Reg
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(!0)),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Reg.to_class(bank),
                    ClassKind::Limit32.to_class(bank),
                    ClassKind::Limit16.to_class(bank),
            ...
            // Stack
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(stack_regs()),
                subclasses: RegClassSet::from_iter([ClassKind::Stack.to_class(bank)]),
                allocation_order: [].to_vec(),
            });
            // Limit32
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(32)),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Limit32.to_class(bank),
                    ClassKind::Limit16.to_class(bank),
                    ClassKind::Limit8.to_class(bank),
                ...
                allocation_order: regs(32).collect(),
            });
            // Limit16
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(16)),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Limit16.to_class(bank),
                    ClassKind::Limit8.to_class(bank),
                    ClassKind::Limit4.to_class(bank),
                ...
                allocation_order: regs(16).collect(),
            });
            // Limit8
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(8)),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Limit8.to_class(bank),
                    ClassKind::Limit4.to_class(bank),
                    ClassKind::Limit2.to_class(bank),
                ]),
                allocation_order: regs(8).collect(),
            });
            // Limit4
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(4)),
                subclasses: RegClassSet::from_iter([
                    ClassKind::Limit4.to_class(bank),
                    ClassKind::Limit2.to_class(bank),
                ]),
                allocation_order: regs(4).collect(),
            });
            // Limit2
            self.classes.push(RegClassData {
                regs: PhysRegSet::from_iter(regs(2)),
                subclasses: RegClassSet::from_iter([ClassKind::Limit2.to_class(bank)]),
                allocation_order: regs(2).collect(),
            });
        }
            ...
            self.operands
                .extend(func.inst_operands(inst).iter().map(|op| {
                    ...
                    let constraint = match op.constraint() {
                        ...
                        crate::OperandConstraint::Stack => {
                            OperandConstraint::Class(ClassKind::Stack.to_class(bank))
                        }
                        ...
                        crate::OperandConstraint::Limit(n) => match n.ilog2() {
                            0 => OperandConstraint::Fixed(PhysReg::new(
                                PReg::new(0, op.class()).index(),
                            )),
                            1 => OperandConstraint::Class(ClassKind::Limit2.to_class(bank)),
                            2 => OperandConstraint::Class(ClassKind::Limit4.to_class(bank)),
                            3 => OperandConstraint::Class(ClassKind::Limit8.to_class(bank)),
                            4 => OperandConstraint::Class(ClassKind::Limit16.to_class(bank)),
                            5 => OperandConstraint::Class(ClassKind::Limit32.to_class(bank)),
                            // There are only 64 registers per bank in regalloc2
                            _ => OperandConstraint::Class(ClassKind::Reg.to_class(bank)),
                        },
                    };
...
impl Regalloc3Ctx {
    pub fn run<F: crate::Function>(
        ...
        if cfg!(debug_assertions) {
            regalloc3::debug_utils::validate_reginfo(&self.reginfo).unwrap();
            regalloc3::debug_utils::validate_function(&self.adapter, &self.reginfo).unwrap();
        }

        // SplitStrategy::Spill seems to work better for most Cranelift
        // workloads: it improves compilation times significantly while not
        // slowing down the generated code.
        let mut options = Options::default();
        options.split_strategy = SplitStrategy::Spill;

        let ra3_output = self
            .regalloc
            .allocate_registers(&self.adapter, &self.reginfo, &options)
